var Stamboom = (function () {
  ...
  Stamboom.prototype.select = function (id) {
    ...
    if (document.getElementById("stamboom_" + id)) {
      ...
      document.getElementById("stamboom_" + id).scrollIntoView({
        behavior: "smooth",
        inline: "center",
        block: "center",
      });
  ...
  function draw() {
      ...
      draw: function (person) {
        return (
          ...
          (selectedPerson == person.id
            ? `style="filled,bold",fillcolor="white",`
            : ``) +
    ...
    dot =
      ...
      `\n` +
      `\n// ----- Siblings -----\n` +
      `{rank=same;ParentsChildrenDot${family.siblings
        .map(function (sibling, x) {
          return `;Sibling${x}Dot`;
        })
        .join("")}}` +
      `\n` +
      `ParentsChildrenDot` +
      ...
      `\n` +
      `\n// ----- Relations -----\n` +
      `{rank=same;Current${family.relations
        .map(function (relation, x) {
          return `;Relation${x}Dot;Relation${x}`;
        })
        .join("")}${family.siblings
        .map(function (sibling, x) {
          return `;Sibling${x}`;
        })
        .join("")}}` +
          ...
          family.relations
            ...
            .map(function (relation, x) {
              return (
                ...
                `Relation${x} -> Relation${
                  x + 1
                }Dot [color="gray",style="dashed"]` +
                `\n` +
                `Relation${x + 1}Dot -> Relation${
                  x + 1
      ...
      (family.relations || [])
        .map(function (relation, x) {
          if (relation.children.length > 0) {
            return (
              `` +
              `{rank=same${relation.children
                .map(function (child, y) {
                  return `;Relation${x}Child${y}Dot`;
                })
                .join("")}}` +
              `\n` +
              `{rank=same${relation.children
                .map(function (child, y) {
                  return `;Relation${x}Child${y}`;
                })
                .join("")}}` +
              `\n` +
              `Relation${x}Dot -> Relation${x}Child${
                relation.children.length - 1
              }Dot` +
              `\n` +
              relation.children
                .map(function (child, y) {
    ...
    hpccWasm.graphviz
      .layout(dot, "svg", "dot", {
        images: [{ path: "img/unknown.png", width: "512px", height: "682px" }],
      })
      .then(function (svg) {
        ...
        if (this.document.getElementsByClassName("selectedPerson")[0]) {
          this.document
            .getElementsByClassName("selectedPerson")[0]
            .scrollIntoView({
              behavior: "auto",
              inline: "center",
              block: "center",
