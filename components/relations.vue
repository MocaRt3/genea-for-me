  ...
  <div class="container">
    <div class="row" style="margin-bottom: 0">
      <div class="col s12 offset-m1">
        <h1 class="header">Parents</h1>
      </div>
    </div>
    <div class="row">
      <div class="col s12 offset-m1">
        <ul
          v-if="person.parents && person.parents.length > 0"
          class="collection with-header">
          <li
            v-for="(parent, index) in person.parents"
    ...
    <div class="row" style="margin-bottom: 0">
      <div class="col s12 offset-m1">
        <h1 class="header">Relations and Children</h1>
      </div>
    </div>
    <div class="row">
      <div class="col s12 offset-m1">
        <template v-for="(relation, index) in person.relations">
          <ul class="collection with-header" :key="relation.id">
            <li class="collection-header">
              <h4>
                <a
                  :href="'#/relations/' + relation.partner.id"
                  class="black-text">
                  {{ relation.partner.caption }}
                </a>
                <a
                  style="float: right"
                  v-on:click="unlinkPartner(relation.id)"
                  v-if="relation.children.length == 0">
                  <i class="material-icons">link_off</i>
                </a>
              </h4>
            </li>
            <li
              v-for="(child, childIndex) in relation.children"
              :key="child.id"
              class="collection-item">
              <a :href="'#/relations/' + child.id">{{ child.caption }}</a>
              <a class="secondary-content" v-on:click="unlinkChild(child.id)">
                <i class="material-icons">link_off</i>
              </a>
            </li>
          </ul>
...
export default {
  ...
  methods: {
    ...
    unlinkChild: function (childId) {
      if (
        window.confirm(
          "Confirm you want to remove this child from the relation."
        )
      ) {
